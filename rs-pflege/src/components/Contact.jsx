...
export default function Contact({ darkMode, lang, cart = [], setCart }) {
    ...
    const totalPrice = cart.reduce((s, i) => s + (Number(i.price) || 0), 0);
    ...
    const sendEmail = async (e) => {
        ...
        const cartContent = cart.length > 0
            ? cart.map(item => `• ${item.name} (${item.carModel || 'Kein Modell'}) - ${item.price}€`).join('\n')
            : "Direktanfrage (Leerer Warenkorb)";

        // Wir erstellen ein FormData Objekt für Web3Forms
        const formData = new FormData(form.current);

        // Dein Access Key von Web3Forms hier eintragen:
        formData.append("access_key", "8da57c53-35df-4746-8217-578703403586");
        formData.append("subject", `Neue Terminanfrage von ${form.current.user_name.value}`);
        formData.append("Warenkorb_Details", cartContent);
        formData.append("Gesamtpreis", `${totalPrice}€`);
        formData.append("from_name", "RS Pflege Website");
        ...
        try {
            ...
            if (data.success) {
                ...
                throw new Error('Fehler beim Provider');
            }
        } catch (err) {
    ...
    return (
        <section id="kontakt" className="py-24 px-6 scroll-mt-20">
            <div className="max-w-4xl mx-auto text-center">
                ...
                <div className={`apple-glass p-8 md:p-12 rounded-[3.5rem] border backdrop-blur-2xl transition-all duration-700 ${darkMode ? 'bg-white/5 border-white/10 shadow-2xl shadow-black/50' : 'bg-black/5 border-black/5 shadow-xl'}`}>
                    ...
                    <AnimatePresence>
                        {cart && cart.length > 0 && (
                            ...
                            >
                                <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest mb-3 italic">Ihre Auswahl:</p>
                                {cart.map((item, idx) => (
                                    <div key={idx} className={`text-[11px] font-bold uppercase py-1 flex justify-between border-b border-blue-500/5 last:border-0 ${darkMode ? 'text-white/80' : 'text-black/70'}`}>
                                        <span>{item.name} <span className="text-blue-500/40 text-[9px] ml-2">{item.carModel}</span></span>
                                        <span className="text-blue-500">{item.price}€</span>
                                    </div>
                                ))}
                                <div className="mt-3 pt-3 border-t border-blue-500/20 flex justify-between font-black text-blue-500 uppercase text-xs">
                                    <span>Gesamt</span>
